...
```{R message=FALSE, plot_gd3_vgene_bubbleplot}
...
```{python results = 'hide', de_gd3_vs_dermal}
...
```{R message=FALSE, get_gd3_geneset}
...
```{python get_scanpy_mean_expression}
...
```{R message=FALSE, gellatly_mean_expression_heatmap}
...
```{R message=FALSE, warning=FALSE, gellatly_gsea}
...
```{python results='hold', gellatly_avg_zscore_umap}
